...
import { Mail, Lock, User, ArrowRight, Loader2, ShieldCheck, Zap, CheckCircle2, AlertCircle } from 'lucide-react';
import { useAppContext } from '../context/AppContext';
import { Logo } from './Logo';
import { supabase } from '../context/AppContext';

type LoginProps = { onBack?: () => void; };
...
export const Login: React.FC<LoginProps> = ({ onBack }) => {
  ...
  const [serverStatus] = useState<'online'>('online');
      ...
      <div className="max-w-md w-full relative">
        {onBack && (
          <button onClick={onBack}
            className="absolute -top-16 left-0 p-3 rounded-2xl bg-white dark:bg-white/5 border border-black/5 dark:border-white/10 text-gray-400 hover:text-emerald-500 transition-all">
        ...
        <motion.div layout className="bg-white dark:bg-[#141414] p-8 rounded-[32px] shadow-xl border border-black/5 dark:border-white/5 transition-colors">
          ...
          {isForgot ? (
            <form onSubmit={handleForgotPassword} className="space-y-4">
              ...
              <button type="submit" disabled={loading}
                className="w-full h-14 bg-emerald-500 text-white rounded-2xl font-bold uppercase tracking-widest text-xs shadow-lg shadow-emerald-500/20 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
              ...
              <button type="button" onClick={() => { setIsForgot(false); setError(''); setSuccess(''); }}
                className="w-full text-center text-xs text-gray-400 hover:text-emerald-500 transition-colors font-bold uppercase tracking-widest">
                ← Back to Login
              </button>
            </form>
          ) : (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {error && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-red-500 text-xs font-medium text-center">{error}</motion.p>}

              {isLogin && (
                <div className="text-right">
                  <button type="button" onClick={() => { setIsForgot(true); setError(''); }}
                    className="text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-emerald-500 transition-colors">
                    Forgot password?
                  </button>
                </div>
              )}
              ...
              <button type="submit" disabled={loading}
                className="w-full h-14 bg-emerald-500 text-white rounded-2xl font-bold uppercase tracking-widest text-xs shadow-lg shadow-emerald-500/20 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2 mt-2">
                {loading ? <Loader2 className="animate-spin" size={20} /> : <><span>{isLogin ? 'Login' : 'Create Account'}</span><ArrowRight size={18} /></>}
              </button>
              ...
              <button type="button" onClick={handleGoogleLogin}
                className="w-full h-14 bg-white dark:bg-white/5 border border-black/5 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/10 text-gray-700 dark:text-gray-200 font-bold rounded-2xl transition-all flex items-center justify-center gap-3 text-xs uppercase tracking-widest">
        ...
        <div className="mt-8 flex items-center justify-center gap-6 text-gray-400">
          <div className="flex items-center gap-2"><ShieldCheck size={16} /><span className="text-[10px] font-bold uppercase tracking-widest">Secure Data</span></div>
          <div className="flex items-center gap-2"><Zap size={16} /><span className="text-[10px] font-bold uppercase tracking-widest">AI Powered</span></div>
        </div>
      </div>
    </div>
  );
